...
<table class="shop_table woocommerce-checkout-review-order-table">
	<tbody>
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			?>
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>" data-product="<?php echo $_product->get_id(); ?>">

					<td class="product-thumb">
						<?php printf( '<span style="background-image: url(%s)"></span>', $image_url ); ?>
					</td>
		...
		<?php foreach ( WC()->cart->get_coupons() as $code => $coupon ) : ?>
			<tr class="cart-discount coupon-<?php echo esc_attr( sanitize_title( $code ) ); ?>">
				<th colspan="2"><?php
					...
					if (strpos(strtoupper( $label ), 'PAY NOW') !== false) {
						$label = str_replace('Coupon: ', '', $label);
					}
					echo $label;
				?></th>
