    ...
    <div class="acc_loop_content">
        ...
        <div class="addtocart_loop_ar">
            ...
            <?php if ($product && $product->is_purchasable() && $product->is_in_stock()) : ?>
                <form class="cart cart_payment_loop_ar" action="<?php echo esc_url(wc_get_cart_url()); ?>" method="post" enctype='multipart/form-data' data-redirect="false">
                    <div class="quantity_price_ar_flex">
                        <div class="quantity_ar">
                            <?php woocommerce_quantity_input(); ?>
                        </div>
                        <div class="price_arq">
                            <h6> <?php echo wc_price($product->get_price()); ?><span class="vat_ar"> incl.VAT</span></h6>
                        </div>
                    </div>
                    <!-- Custom Add to Cart Button Text -->
                    <button type="submit" name="add-to-cart" value="<?php echo esc_attr($productid); ?>" class="button alt">
                        <?php echo esc_html('Add to Cart'); // Custom button text 
                        ?>
                    </button>
                    <!-- Disable Redirection -->
