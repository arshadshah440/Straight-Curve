	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="product-price"><?php esc_html_e( ra_lang('Price'), 'wc-cart-pdf' ); ?></th>
				<th class="product-quantity"><?php esc_html_e( ra_lang('Quantity'), 'wc-cart-pdf' ); ?></th>
				<th class="product-subtotal"><?php esc_html_e( ra_lang('Total'), 'wc-cart-pdf' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						if (isset($product_info['details']) && $product_info['details']) {
							echo '<div style="display: block; font-size: 10px;"><br>' . $product_info['details'] . '</div>';
						}

						?>
						</td>

						<td class="product-price" data-title="<?php esc_attr_e( ra_lang('Price'), 'wc-cart-pdf' ); ?>">
							<?php
								echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key ); // phpcs:ignore
							?>
						</td>

						<td class="product-quantity" data-title="<?php esc_attr_e( ra_lang('Quantity'), 'wc-cart-pdf' ); ?>">
							<?php print esc_html( $cart_item['quantity'] ); ?>
						</td>

						<td class="product-subtotal" data-title="<?php esc_attr_e( ra_lang('Total'), 'wc-cart-pdf' ); ?>">
							<?php
			...
			<tr class="cart-subtotal cart-total-row">
				<th class="row-subtotal" colspan="4" style="text-align: right;"><?php esc_html_e( ra_lang('Subtotal'), 'wc-cart-pdf' ); ?></th>
				<td class="row-subtotal" data-title="<?php esc_attr_e( ra_lang('Subtotal'), 'wc-cart-pdf' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></td>
			</tr>

			<?php if ( 0 < WC()->cart->get_shipping_total() ) : ?>
				<tr class="shipping cart-total-row">
					<th class="row-subtotal" colspan="4" style="text-align: right;"><?php esc_html_e( ra_lang('Shipping'), 'wc-cart-pdf' ); ?></th>
					<td class="row-subtotal" data-title="<?php esc_attr_e( ra_lang('Shipping'), 'wc-cart-pdf' ); ?>"><?php echo WC()->cart->get_cart_shipping_total(); ?></td>
				</tr>
			<?php endif; ?>
			...
			<tr class="order-total cart-total-row">
				<th class="row-subtotal" colspan="4" style="text-align: right;"><?php esc_html_e( ra_lang('Total'), 'wc-cart-pdf' ); ?></th>
				<td class="row-subtotal" data-title="<?php esc_attr_e( ra_lang('Total'), 'wc-cart-pdf' ); ?>"><?php wc_cart_totals_order_total_html(); ?></td>
			</tr>
